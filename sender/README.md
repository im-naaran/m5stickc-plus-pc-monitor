...
内置操作码处理器中，`OP-CP`、`OP-PA`、`OP-WIN-NEXT` 使用 `pyautogui` 发送跨平台系统快捷键：macOS 使用 Command 组合键，Windows/Linux 使用 Ctrl 或 Alt 组合键。`OP-LOCK` 暂时仍通过 `system_actions.py` 调用 macOS 的 `pmset displaysleepnow`；其他系统可以继续收发指标和页面，但锁屏命令需要补充对应实现。
